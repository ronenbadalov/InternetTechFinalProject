  ...
  const GameModal = ({ landData }) => {
    ...
    return (
        <Container className={classes["container"]} fluid>
            <h3 className={classes["header"]} >{(landData.innerData) ? landData.innerData.name : "No Game"}</h3>
            {(landData.innerData) ? <iframe className={classes["frame"]}  src={landData.innerData.url} /> : ""}
